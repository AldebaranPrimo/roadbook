...
- **B-5 (palliativo) — link a navigatori esterni nascosti su mobile**. In `PuntoCard.vue` i bottoni "Google Maps", "Waze" e "Apple Maps" hanno ora la classe `solo-desktop` e una regola `@media (max-width: 899.98px) { .azioni .solo-desktop { display: none; } }` li nasconde sotto i 900 px. Su mobile resta solo OsmAnd, l'unico provider che funziona correttamente perché usa il deep link ufficiale `https://osmand.net/go?...` (gli altri 3 perdevano la destinazione nello switch web→app, problema documentato in TODO B-5). Su desktop (≥ 900 px) tutti e 4 i bottoni restano invariati. Helper in `mappe-esterne.js` non toccati — restano usati su desktop. Il fix definitivo con deep link nativi corretti (`maps/dir/?api=1&destination=...` per Google, schema custom `waze://` per Waze, `?daddr=` per Apple Maps + UA-detect iOS) è tracciato come voce di backlog TODO #12 e richiederà test sul device reale. Bump `1.1.3 → 1.1.4`.
- **B-4 fase 2 — header su tre righe, titolo non più troncato dal layout a colonne**. La fase 1 (rimozione del bottone ⓘ inline) aveva risolto il problema dell'icona tagliata via dall'ellipsis ma non quello sottostante: `.app-header` era un flex orizzontale con `.titoli` (`flex: 1; min-width: 0`) accanto a `.azioni` (`flex-shrink: 0`), quindi il titolo del viaggio riceveva solo la larghezza residua dopo i quattro pulsanti azione e finiva in ellipsis nonostante la metà destra dello schermo restasse visivamente vuota nelle righe sottostanti. Il fix riscrive `HeaderApp.vue` per mettere logo e azioni in una prima riga (`.header-riga-marchio` con `display: flex; justify-content: space-between`) e titolo + sottotitolo in due righe successive a tutta larghezza dell'header. In parallelo `.btn-ghost` in `src/styles/app.css` passa da `height: 28px` a `height: 24px` (font `0.78rem → 0.72rem`, padding `0 0.5rem → 0 0.45rem`) per stare nella stessa altezza visiva del marchio sulla prima riga e non espandere l'header. Bump `1.1.4 → 1.1.5`.

### Tooling

- **Hook `SessionStart` per check MCP Playwright** in `.claude/settings.json` (nuovo file, primo settings di repo per Claude Code in questa repo). All'avvio di ogni sessione esegue `claude mcp list` (PowerShell, timeout 30 s) e cerca `playwright.*Connected`: se il MCP non risulta attivo, emette un `systemMessage` che avvisa Claude di segnalare subito all'utente la mancanza prima di accettare task UI / smoke test. Il check fallisce silenziosamente in modo non bloccante se `claude mcp list` errora. Motivazione: il 2026-04-27 una sessione ha pianificato e implementato fix UI promettendo smoke test Playwright, salvo poi scoprire a metà che il MCP non era caricato — perdita di tempo prevenibile con verifica a inizio sessione. `.gitignore` aggiornato per escludere `.claude/settings.local.json` (override personali futuri non finiscono in repo).
